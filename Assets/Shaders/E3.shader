    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float xSin = (sin(i.uv.x*100-_Time*100)*0.5+0.5);
                float xSin2 = (sin(i.uv.x*100+offset-_Time*100)*0.5+0.5);
                float xSin3 = (sin(i.uv.x*100+2*offset-_Time*100)*0.5+0.5);
                float xSin4 = (sin(i.uv.x*100+3*offset-_Time*100)*0.5+0.5);


                //draw a sin wave
                if(abs(i.uv.y - xSin) < _thickness && i.uv.x < _lerp.x){
                    return fill;
                }

                if(abs(i.uv.y - xSin2) < _thickness && i.uv.x < _lerp.x){
                    return fixed4(0,1,0,1);
                }

                if(abs(i.uv.y - xSin3) < _thickness && i.uv.x < _lerp.x){
                    return fixed4(0,0,1,1);
                }

                if(abs(i.uv.y - xSin4) < _thickness && i.uv.x < _lerp.x){
